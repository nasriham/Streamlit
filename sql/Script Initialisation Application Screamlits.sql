...
create or replace view DB_REFERENTIEL_DEV.S_REFERENTIEL.V_PARC_CAPACITE_DYNAMIQUE(
	CODE_PARC,
	NOM_PARC,
...
SELECT
    ...
    p.NOM_PARC,
    COALESCE(SUM(
        COALESCE(e.NB_CAPACITE_VL_DT_THERMIQUE, 0) +
        COALESCE(e.NB_CAPACITE_VL_DT_ELECTRIQUE, 0) +
        COALESCE(e.NB_CAPACITE_VL_DT_PMR, 0) +
        COALESCE(e.NB_CAPACITE_VL_DT_PMR_ELECTRIQUE, 0) +
        COALESCE(e.NB_CAPACITE_VL_DT_MOTO, 0) +
        COALESCE(e.NB_CAPACITE_VL_DT_VELO, 0) +
        COALESCE(e.NB_CAPACITE_VL_DT_AUTRE, 0)
    ), 0) AS NB_CAPACITE_EXPLOITEE,
    COALESCE(SUM(
        COALESCE(e.NB_CAPACITE_VL_DT_THERMIQUE, 0) +
        COALESCE(e.NB_CAPACITE_VL_DT_ELECTRIQUE, 0) +
        COALESCE(e.NB_CAPACITE_VL_DT_PMR, 0) +
        ...
        COALESCE(e.NB_CAPACITE_VL_DT_VELO, 0) +
        COALESCE(e.NB_CAPACITE_VL_DT_AUTRE, 0)
    ), 0) AS NB_CAPACITE_TOTALE,
    COALESCE(eq.NB_VOIES_ENTREES, 0) AS NB_PISTES_ENTREE,
    COALESCE(eq.NB_VOIES_SORTIES, 0) AS NB_PISTES_SORTIE,
    COUNT(e.NIVEAU) AS NB_NIVEAUX
FROM DB_REFERENTIEL_DEV.S_REFERENTIEL.T_R_PARC p
LEFT JOIN DB_REFERENTIEL_DEV.S_REFERENTIEL.T_R_PARC_EXPLOITATION e
    ON p.CODE_PARC = e.CODE_PARC AND e.IS_ACTIVE = TRUE
LEFT JOIN DB_REFERENTIEL_DEV.S_REFERENTIEL.T_R_EQUIPEMENT_ACCES eq
    ON p.CODE_PARC = eq.CODE_PARC AND eq.IS_ACTIVE = 'TRUE'
WHERE p.IS_ACTIVE = 'TRUE'
GROUP BY p.CODE_PARC, p.NOM_PARC, eq.NB_VOIES_ENTREES, eq.NB_VOIES_SORTIES
ORDER BY p.NOM_PARC;
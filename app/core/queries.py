...
def get_type():
    return session.sql("""
        SELECT CODE_TYPE_PARC,
               LIBELLE_TYPE_PARC  
        FROM S_REFERENTIEL.T_TYPE_PARC
    """).to_pandas()
...
def get_nature_juridique():
    return session.sql("""
        ...
        FROM S_REFERENTIEL.T_NATURE_JURIDIQUE
    """).to_pandas()
...
def get_parks():
    return session.sql("""
        SELECT 
            ...
            DATE_FIN,
            IS_ACTIVE
        FROM S_REFERENTIEL.T_BASE_PARC
...
def get_parks_exploit():
    return session.sql("""
        SELECT *
        FROM S_REFERENTIEL.T_BASE_PARC_EXPLOITATION
        WHERE IS_ACTIVE = TRUE
    """).to_pandas()
...
def get_parks_commercial():
    return session.sql("""
        SELECT
            ...
            DATE_FIN,
            IS_ACTIVE
        FROM S_REFERENTIEL.T_BASE_PARC_COMMERCIAL
        WHERE IS_ACTIVE = TRUE
        ORDER BY CODE_PARC
    """).to_pandas()
...
def get_parks_juridique():
    return session.sql("""
        ...
        DATE_FIN,
        IS_ACTIVE
        FROM S_REFERENTIEL.T_BASE_PARC_JURIDIQUE
        WHERE IS_ACTIVE = TRUE
        ORDER BY CODE_PARC
    """).to_pandas()
...
def get_parks_incendie():
    return session.sql("""
        SELECT 
            ...
            DATE_FIN,
            IS_ACTIVE
        FROM S_REFERENTIEL.T_BASE_PARC_SECURITE_INCENDIE
        WHERE IS_ACTIVE = TRUE
    """).to_pandas()
...
def get_parks_divers():
    return session.sql("""
        SELECT 
            ...
            DATE_FIN,
            IS_ACTIVE
        FROM S_REFERENTIEL.T_BASE_PARC_DIVERS
        WHERE IS_ACTIVE = TRUE
    """).to_pandas()
...
def insert_park(row):
    session.sql("""
        INSERT INTO S_REFERENTIEL.T_BASE_PARC (
            CODE_PARC,
            NOM_PARC,
            ADRESSE_PARC,
            CODE_VILLE,
...
def insert_exploit(row: dict, user: str):
    session.sql("""
        INSERT INTO S_REFERENTIEL.T_BASE_PARC_EXPLOITATION (
            CODE_PARC,
            NB_CAPACITE_TOTALE,
...
def insert_park_commercial(row, user):
    session.sql("""
        INSERT INTO S_REFERENTIEL.T_BASE_PARC_COMMERCIAL (
            CODE_PARC,
            CODE_FLAG_HORAIRES,
            CODE_FLAG_ABONNES,
            CODE_FLAG_AMODIES,
...
def insert_park_juridique(row, user):
    session.sql("""
        INSERT INTO S_REFERENTIEL.T_BASE_PARC_JURIDIQUE (
            CODE_PARC,
            MISE_EN_SERVICE,
            CODE_NATURE_JURIDIQUE,
            SIRET,	
...
def insert_incendie(row: dict, user: str):
    ...
    session.sql("""
        INSERT INTO S_REFERENTIEL.T_BASE_PARC_SECURITE_INCENDIE (
            CODE_PARC,
            EAE_SPRINKLEURS,
            NB_POSTES,
            NB_TETES,
...
def insert_park_divers(row: dict, user: str):
    ...
    session.sql("""
        INSERT INTO S_REFERENTIEL.T_BASE_PARC_DIVERS (
            CODE_PARC,
            GABARIT_STANDARD,
            CODE_FLAG_PRESENCE_HUMAINE,
            NB_ASCENCEURS,
...
def update_park(row):
    ...
    session.sql("""
        UPDATE S_REFERENTIEL.T_BASE_PARC
        SET 
...
def update_exploit(row: dict, user: str):
    ...
    session.sql("""
        UPDATE S_REFERENTIEL.T_BASE_PARC_EXPLOITATION
        SET 
            DATE_FIN = CURRENT_TIMESTAMP(),
            IS_ACTIVE = FALSE,
            NOM_MODIFICATEUR = ?,
...
def update_commercial(row, user):
    ...
    # 1. close ancienne ligne
    session.sql("""
        UPDATE S_REFERENTIEL.T_BASE_PARC_COMMERCIAL
        SET 
            DATE_FIN = CURRENT_TIMESTAMP(),
            IS_ACTIVE = FALSE,
            NOM_MODIFICATEUR = ?,
...
def update_juridique(row, user):
    ...
    # 1. close ancienne ligne
    session.sql("""
        UPDATE S_REFERENTIEL.T_BASE_PARC_JURIDIQUE
        SET 
            DATE_FIN = CURRENT_TIMESTAMP(),
            IS_ACTIVE = FALSE,
            NOM_MODIFICATEUR = ?,
...
def update_incendie(row: dict, user: str):
    ...
    # 1. close ancienne ligne
    session.sql("""
        UPDATE S_REFERENTIEL.T_BASE_PARC_SECURITE_INCENDIE
        SET 
            DATE_FIN = CURRENT_TIMESTAMP(),
            IS_ACTIVE = FALSE,
            NOM_MODIFICATEUR = ?,
...
def update_divers(row, user):
    ...
    # 1. close ancienne ligne
    session.sql("""
        UPDATE S_REFERENTIEL.T_BASE_PARC_DIVERS
        SET 
            DATE_FIN = CURRENT_TIMESTAMP(),
            IS_ACTIVE = FALSE,
            NOM_MODIFICATEUR = ?,
